...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      showSuccess('Registration completed successfully! Welcome to StockPilot.');
      navigate('/dashboard');
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-200">
      ...
      <div className="w-full max-w-md p-8 sm:p-10 mx-4 z-10 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl rounded-2xl transition-all duration-200">
        <div className="text-center mb-8 select-none caret-transparent">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Join the StockPilot Workspace</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border ${
            ...
            />
            {errors.name && <p className="mt-1 text-xs font-medium text-rose-500 dark:text-rose-400">{errors.name}</p>}
          </div>
          ...
          <div>
            ...
            />
            {errors.email && <p className="mt-1 text-xs font-medium text-rose-500 dark:text-rose-400">{errors.email}</p>}
          </div>
          ...
          <div>
            ...
            />
            {errors.password && <p className="mt-1 text-xs font-medium text-rose-500 dark:text-rose-400">{errors.password}</p>}
          </div>
          ...
          <div>
            ...
            <select
              ...
              value={formData.role}
              onChange={handleChange}
              className="w-full px-4 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white focus:outline-hidden focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all cursor-pointer"
            >
              <option value="staff" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-white">Staff Member</option>
              <option value="admin" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-white">Administrator</option>
            </select>
          </div>
          ...
          >
            {submitting ? <Spinner size="sm" color="white" /> : 'Register'}
          </button>
        </form>
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            ...
            <Link to="/login" className="font-semibold text-sky-500 hover:text-sky-600 dark:text-sky-400 dark:hover:text-sky-300 hover:underline">
              Sign In
            </Link>
          </p>
        </div>
